...
class PowerLoraLoaderHeaderWidget extends RgthreeBaseWidget {
    constructor(name = "PowerLoraLoaderHeaderWidget") {
        ...
        this.hitAreas = {
            ...
            presets: { bounds: [0, 0], onDown: this.onPresetsDown },
        };
    ...
    draw(ctx, node, w, posY, height) {
        ...
        if (!lowQuality) {
            ...
            const toggleText = "Toggle All";
            ctx.fillText(toggleText, posX, midY);
            const toggleWidth = ctx.measureText(toggleText).width;

            // Draw Presets button
            const presetsText = "Presets ▼";
            const presetsWidth = ctx.measureText(presetsText).width;
            const presetsX = posX + toggleWidth + innerMargin * 2;
            ctx.fillText(presetsText, presetsX, midY);

            this.hitAreas.presets.bounds = [presetsX, presetsWidth];
    ...
    onPresetsDown(event, pos, node) {
        getPresets().then((presets) => {
            const menuItems = [];

            // Load Presets
            if (presets.length > 0) {
                 const loadItems = presets.map(p => ({
                    content: p.name,
                    callback: () => {
                        this.loadPreset(node, p);
                    }
                }));
                menuItems.push({
                    content: "📂 Load Preset",
            ...
            menuItems.push({
             ...
             // Delete Presets
            if (presets.length > 0) {
                 const deleteItems = presets.map(p => ({
                    content: `🗑️ ${p.name}`,
                    callback: () => {
                        if (confirm(`Are you sure you want to delete preset "${p.name}"?`)) {
                            deletePreset(p.name).then(() => {
                                alert(`Deleted preset "${p.name}"`);
                            });
                        }
                    }
                }));
    ...
    loadPreset(node, preset) {
        ...
        for (const w of widgetsToRemove) {
            removeArrayItem(node.widgets, w);
        }

        // Reset counter? Not strictly necessary but clean.
        // node.loraWidgetsCounter = 0; // Don't reset, might conflict if we keep unique names.

        // Add new widgets
        for (const loraData of preset.loras) {
            const widget = node.addNewLoraWidget(loraData.lora);
            widget.value = { ...loraData };
        }

        node.setDirtyCanvas(true, true);
    }

    savePreset(node, name) {
        const loras = [];
        for (const w of node.widgets) {
             if (w.name && w.name.startsWith("lora_")) {
                 loras.push(w.value);
